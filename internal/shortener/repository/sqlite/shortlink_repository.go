...
func (r *shortLinkRepository) DeleteByCode(ctx context.Context, code string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM short_links WHERE code = ?`, code)
	if err != nil {
		return err
	}
	n, _ := result.RowsAffected()
...
func (r *shortLinkRepository) SetActiveByCode(ctx context.Context, code string, active bool) error {
	result, err := r.db.ExecContext(ctx, `UPDATE short_links SET is_active = ? WHERE code = ?`, active, code)
	if err != nil {
		return err
	}
	n, _ := result.RowsAffected()
