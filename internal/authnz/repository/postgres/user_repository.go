...
func (u *userRepository) FindByUsername(ctx context.Context, username string) (model.User, error) {
	var user model.User
	if err := u.db.GetContext(ctx, &user, "SELECT * FROM users WHERE username = $1", username); err != nil {
...
func (u *userRepository) Find(ctx context.Context, id uuid.UUID) (model.User, error) {
	var user model.User
	if err := u.db.GetContext(ctx, &user, "SELECT * FROM users WHERE id = $1", id.String()); err != nil {
		return model.User{}, err
	}
	return user, nil
}
