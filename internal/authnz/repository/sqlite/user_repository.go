...
func (u *userRepository) FindByUsername(ctx context.Context, username string) (model.User, error) {
	var user model.User
	if err := u.db.GetContext(ctx, &user, "SELECT "+selectColumns+" FROM users WHERE username = ?", username); err != nil {
...
func (u *userRepository) Find(ctx context.Context, id uuid.UUID) (model.User, error) {
	var user model.User
	if err := u.db.GetContext(ctx, &user, "SELECT "+selectColumns+" FROM users WHERE id = ?", id.String()); err != nil {
		return model.User{}, err
	}
	return user, nil
}
