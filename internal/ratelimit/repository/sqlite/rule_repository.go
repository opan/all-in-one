...
func (r *ruleRepository) Get(ctx context.Context, targetKey string) (model.Rule, error) {
	var rule model.Rule
	if err := r.db.GetContext(ctx, &rule, "SELECT * FROM rate_limit_rules WHERE target_key = ?", targetKey); err != nil {
...
func (r *ruleRepository) Seed(ctx context.Context, rule model.Rule, opts ...query.QueryOptions) error {
	...
	_, err := exec.NamedExecContext(ctx,
		`INSERT OR IGNORE INTO rate_limit_rules (target_key, enabled, limit_count, window_value, window_unit, updated_at, updated_by)
