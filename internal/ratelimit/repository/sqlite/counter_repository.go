...
func (r *counterRepository) IncrAndGet(ctx context.Context, targetKey, bucketKey, day string) (int, error) {
	...
	err := r.db.QueryRowxContext(ctx,
		...
		RETURNING count`,
		targetKey, bucketKey, day, now, now).Scan(&count)
	return count, err
}
...
func (r *counterRepository) DeleteForTargetDay(ctx context.Context, targetKey, day string) error {
	...
	_, err := r.db.ExecContext(ctx, "DELETE FROM rate_limit_counters WHERE target_key = ? AND day = ?", targetKey, day)
	return err
}

func (r *counterRepository) DeleteOlderThan(ctx context.Context, day string) (int64, error) {
	res, err := r.db.ExecContext(ctx, "DELETE FROM rate_limit_counters WHERE day < ?", day)
	if err != nil {
		return 0, err
	}
