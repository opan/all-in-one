...
func (r *featureRepository) GetByKey(ctx context.Context, key string) (model.Feature, error) {
	...
	if err := r.db.GetContext(ctx, &feature, "SELECT * FROM features WHERE key = ?", key); err != nil {
...
func (r *featureRepository) Upsert(ctx context.Context, feature model.Feature) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT OR IGNORE INTO features (id, key, name, description, admin_only, created_at, updated_at)
