...
func (r *userGroupRepository) GetGroupID(ctx context.Context, userID uuid.UUID) (*uuid.UUID, error) {
	...
	err := r.db.GetContext(ctx, &groupID, "SELECT group_id FROM users WHERE id = ?", userID.String())
	if err != nil {
...
func (r *userGroupRepository) AssignGroup(ctx context.Context, userID uuid.UUID, groupID *uuid.UUID, opts ...query.QueryOptions) error {
	...
	_, err := exec.ExecContext(ctx, "UPDATE users SET group_id = ? WHERE id = ?", groupIDValue, userID.String())
	return err
}

func (r *userGroupRepository) CountByGroup(ctx context.Context, groupID uuid.UUID) (int, error) {
	var count int
	err := r.db.GetContext(ctx, &count, "SELECT COUNT(*) FROM users WHERE group_id = ?", groupID.String())
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (r *userGroupRepository) ListUsersWithGroup(ctx context.Context) ([]model.UserAccessRow, error) {
	...
	err := r.db.SelectContext(ctx, &rows, `
		SELECT users.id AS user_id, users.username, COALESCE(users.email, '') AS email, users.group_id, users.blocked, groups.name AS group_name
		FROM users
